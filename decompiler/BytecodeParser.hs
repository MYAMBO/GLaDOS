...
data Lit = Lit
  { lty   :: String   -- e.g. "Int32", "Bool", "Float", ...
  , lshow :: String   -- e.g. "5"
  } deriving (Eq, Show)
...
------------------------------------------------------------
-- Normalization (clean up negation patterns etc.)
------------------------------------------------------------
...
------------------------------------------------------------
-- Type inference
-- Goal: infer param types and return types in a generic way.
------------------------------------------------------------

-- A generic type slot. Either we know it's "Int32", "Bool", etc.,
-- or we don't know.
data Ty
  = TyNamed String   -- e.g. "Int32", "Bool", "Float", ...
  | TyUnknown
  deriving (Eq, Show)
...
mergeTy (TyNamed a) (TyNamed b)
  | a == b    = TyNamed a
  | otherwise = TyUnknown  -- conflict, can't be both
                           -- -> degrade to unknown

-- Some ops are arithmetic/bool-ish.
-- We don't hardcode Int32 anymore. We'll infer from kids.
-- If kids agree on a type, result has that type.
-- For comparisons (Eq, Lt, etc.) the result is Bool.
...
inferExprType e =
  case e of
    ...
    ECall f args ->
      -- we don't know the callee's return type directly.
      -- Heuristic: merge arg types. If they all agree on something non-unknown,
      -- we assume that's also the return type. Otherwise unknown.
      let argTys = map inferExprType args
          merged = foldr mergeTy TyUnknown argTys
      in merged
    ...
    EBin o xs ->
      if isComparison o
        ...
        then
          -- all args merged
          let ts = map inferExprType xs
          in foldr mergeTy TyUnknown ts
      else
        -- fallback: merge children
        let ts = map inferExprType xs
        in foldr mergeTy TyUnknown ts
...
-- Mark direct variables inside an expression with
-- the type of that expression if that expression's type is concrete.
--
-- Example:
--   If we see EBin Add [EVar "a0", EVar "a1"]
--   inferExprType whole binop = TyNamed "Int32"
--   => we assign "a0" -> Int32, "a1" -> Int32
--
-- This is local: it only tags immediate child vars.
inferVarTypes :: Expr -> M.Map String Ty
inferVarTypes e = go e
  where
    markChildren :: Expr -> Ty -> M.Map String Ty
    markChildren ex tyHere = case ex of
      EVar v ->
        case tyHere of
          TyNamed t -> M.singleton v (TyNamed t)
          _         -> M.empty
      _ -> M.empty
    ...
    go expr =
      case expr of
        ...
        EBin _ xs ->
          let tyHere = inferExprType expr
              mine   = mconcat [ markChildren x tyHere | x <- xs ]
              recs   = mconcat (map go xs)
          in M.unionWith mergeTy mine recs

        ECall f xs ->
          let tyHere = inferExprType expr
              mine   = mconcat [ markChildren a tyHere | a <- xs ]
...
-- Return type for the function body.
-- We just ask: what type is this body expression?
...
------------------------------------------------------------
-- Renaming for pretty params
------------------------------------------------------------

-- we turn a0,a1,a2,... into a,b,c,d,...
-- If we run out of letters, we continue with p0,p1,...
prettyParamNames :: Int -> [String]
prettyParamNames argc =
  let base = ["a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z"]
  in if argc <= length base
        then take argc base
        else take (length base) base
          ++ [ "p" ++ show i | i <- [0 .. argc - length base - 1] ]

-- rename vars inside the body expression according to a mapping
renameVars :: M.Map String String -> Expr -> Expr
renameVars m e =
  case e of
    EVar v ->
    ...
    ECall f xs ->
      ECall (renameVars m f) (map (renameVars m) xs)

    EBin o xs ->
      EBin o (map (renameVars m) xs)

    EIf c t f ->
      EIf (renameVars m c) (renameVars m t) (renameVars m f)

------------------------------------------------------------
-- Build TFunc from decoded function body
------------------------------------------------------------

-- This is where we create headers like:
--   func adder<Int32 a, Int32 b> => Int32
-- and also rewrite the body so it uses `a`, `b` instead of `a0`, `a1`.
buildFuncTop :: String -> Int -> Expr -> Top
buildFuncTop fname argc origBodyExpr =
  let
    -- Raw param names from VM
    rawParamNames :: [String]
    rawParamNames = [ "a" ++ show i | i <- [0 .. argc - 1] ]

    -- Pretty param names we want to show (a, b, c, ...)
    prettyNames :: [String]
    prettyNames = prettyParamNames argc

    -- Mapping old -> new (e.g. "a0" -> "a", "a1" -> "b", ...)
    renameMap :: M.Map String String
    renameMap = M.fromList (zip rawParamNames prettyNames)

    -- Rename inside body
    bodyExpr :: Expr
    bodyExpr = renameVars renameMap origBodyExpr

    -- Infer types of vars from the *original* body
    -- (not renamed, so we match "a0", "a1" etc.)
    tyMapRaw :: M.Map String Ty
    tyMapRaw = inferVarTypes origBodyExpr

    -- For each param, figure out its type name, if any
    lookupTyName :: String -> Maybe String
    lookupTyName rawName = case M.lookup rawName tyMapRaw of
      Just (TyNamed t) -> Just t
      _                -> Nothing

    finalParams :: [Param]
    finalParams =
      [ Param { pType = lookupTyName raw
              , pName = pretty
              }
      | (raw, pretty) <- zip rawParamNames prettyNames
      ]

    retTy = inferReturnTypeName origBodyExpr
...
------------------------------------------------------------
-- Parse env (top-level function defs live here in v2)
------------------------------------------------------------
...
parseEnvValAsTop name c0 = do
  ...
  case tag of
    t | t == tagFunc -> do
      ...
      | t == tagOp -> do
          -- Ignore raw ops in env
          (_, c2) <- getU8 c1
          pure (Nothing, c2)

      | otherwise -> do
          -- Other env constants are ignored
...
------------------------------------------------------------
-- Expression decoder (byte-count jumps)
------------------------------------------------------------
...
data Est = Est
  { stk    :: [SI]
  , maxArg :: Int  -- max index of any aN seen
  } deriving (Show)
...
renderProgram tops =
  ...
  where
    -- Put functions first, then vars, then loose exprs
    ordered =
      [ t | t@TFunc{} <- tops ] ++
      [ t | t@TVar{}  <- tops ] ++
      [ t | t@TExpr{} <- tops ]
