...
tests = testGroup "Interpret tests"
  [ testGroup "basic arithmetic"
      [ testCase "(+ 1 2 3) = 6" $
          eval [] example1 @?= Just (Just (Atom 6), [])
      , testCase "(+ 5 10) = 15" $
          eval [] (List [Symbol "+", Atom 5, Atom 10]) @?= Just (Just (Atom 15), [])
      , testCase "example2 with define and -" $
          eval [] example2 @?= Just (Just (Atom 37), [("hello", Atom 42)])
      , testCase "(- 2 3) = -1" $
          eval [] (List [Symbol "-", Atom 2, Atom 3]) @?= Just (Just (Atom (-1)), [])
      , testCase "(* 2 4) = 8" $
          eval [] example3 @?= Just (Just (Atom 8), [])
      , testCase "(div 6 2) = 3" $
          eval [] example4 @?= Just (Just (Atom 3), [])
      , testCase "(mod 10 4) = 2" $
          eval [] example6 @?= Just (Just (Atom 2), [])
      ]
  , testGroup "boolean comparisons"
      [ testCase "(> 10 4)" $
          eval [] example8 @?= Just (Just (ABool True), [])
      , testCase "(> 0 10)" $
          eval [] example9 @?= Just (Just (ABool False), [])
      , testCase "(< 0 4)" $
          eval [] example10 @?= Just (Just (ABool True), [])
      , testCase "(< 10 0)" $
          eval [] example11 @?= Just (Just (ABool False), [])
      , testCase "(eq? 0 0)" $
          eval [] example12 @?= Just (Just (ABool True), [])
      , testCase "(eq? 10 0)" $
          eval [] example13 @?= Just (Just (ABool False), [])
      ]
  , testGroup "define and variables"
      [ testCase "(define x 5)" $
          eval [] (Define "x" (Atom 5)) @?= Just (Nothing, [("x", Atom 5)])
      , testCase "lookup after define" $
          let env = [("x", Atom 5)]
          in eval env (Symbol "x") @?= Just (Just (Atom 5), env)
      , testCase "exampleDefineTest" $
          eval [] exampleDefineTest @?= Just (Just (Atom 42), [("test", Atom 42)])
      ]
  , testGroup "if expressions"
      [ testCase "(if #t 1 2)" $
          eval [] (If (ABool True) (Atom 1) (Atom 2)) @?= Just (Just (Atom 1), [])
      , testCase "(if #f 1 2)" $
          eval [] (If (ABool False) (Atom 1) (Atom 2)) @?= Just (Just (Atom 2), [])
      , testCase "exampleTest" $
          eval [] exampleTest @?= Just (Just (Atom 1), [])
      ]
  , testGroup "atoms and bools"
      [ testCase "Atom 42" $
          eval [] (Atom 42) @?= Just (Just (Atom 42), [])
      , testCase "ABool True" $
          eval [] (ABool True) @?= Just (Just (ABool True), [])
      ]
  , testGroup "lambda expressions"
      [ testCase "lambda definition" $
          let lam = Lambda ["x"] (Atom 1)
          in eval [] lam @?= Just (Just lam, [])
      , testCase "lambda application (example14)" $
          eval [] example14 @?= Just (Just (Atom 5), [])
      , testCase "exampleDefine" $
          let expected = Lambda ["x","y"] (List [Symbol "+", Symbol "x", Symbol "y"])
          in eval [] exampleDefine @?= Just (Nothing, [("caca", expected)])
      , testCase "exampleMultiDefine" $
          ...
          in eval [] exampleMultiDefine @?= Just (Nothing, env)
      ]
  , testGroup "call expressions"
      [ testCase "builtin operators" $
          eval [] (Call (Symbol "+") [Atom 1, Atom 2]) @?= Just (Just (Atom 3), [])
      , testCase "user-defined function" $
          let env = [("add", Lambda ["x","y"] (List [Symbol "+", Symbol "x", Symbol "y"]))]
          in eval env (Call (Symbol "add") [Atom 2, Atom 3]) @?= Just (Just (Atom 5), env)
      , testCase "exampleDefineCall" $
          let helloLam = Lambda ["x"] (Call (Symbol "+") [Symbol "x", Atom 1])
          in eval [] exampleDefineCall @?= Just (Just (Atom 3), [("hello", helloLam)])
      ]
  , testGroup "complex lambda examples"
      ...
      , testCase "apply exampleLambdaWithIf positive" $
          ...
          in eval [] app @?= Just (Just (Atom 5), [])
      , testCase "apply exampleLambdaWithIf negative" $
          let app = List [exampleLambdaWithIf, Atom (-3)]
          in eval [] app @?= Just (Just (Atom 0), [])
      ]
  ]

-- reuse your example definitions
