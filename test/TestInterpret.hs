...
spec = do
  describe "basic arithmetic" $ do
    it "evaluates (+ 1 2 3) = 6" $ do
      eval [] example1 `shouldBe` Right (Just (Atom 6), [])

    it "evaluates (+ 5 10) = 15" $ do
      eval [] (List [Symbol "+", Atom 5, Atom 10]) `shouldBe` Right (Just (Atom 15), [])

    it "evaluates example2 with define and subtraction" $ do
      let expected = Right (Just (Atom 37), [("hello", Atom 42)])
      eval [] example2 `shouldBe` expected

    it "evaluates (- 2 3) = -1" $ do
      eval [] (List [Symbol "-", Atom 2, Atom 3]) `shouldBe` Right (Just (Atom (-1)), [])

    it "evaluates (* 2 4) = 8" $ do
      eval [] example3 `shouldBe` Right (Just (Atom 8), [])

    it "evaluates (div 6 2) = 3" $ do
      eval [] example4 `shouldBe` Right (Just (Atom 3), [])

--    it "evaluates (div 6 0) = error call" $ do
--      evaluate (eval [] example5) `shouldThrow` anyErrorCall

    it "evaluates (mod 10 4) = 2" $ do
      eval [] example6 `shouldBe` Right (Just (Atom 2), [])
...
--      evaluate (eval [] example7) `shouldThrow` anyErrorCall
  ...
  describe "boolean comparisons" $ do
    it "evaluates (> 10 4) = True" $ do
      eval [] example8 `shouldBe` Right (Just (ABool True), [])

    it "evaluates (> 0 10) = False" $ do
      eval [] example9 `shouldBe` Right (Just (ABool False), [])

    it "evaluates (< 0 4) = True" $ do
      eval [] example10 `shouldBe` Right (Just (ABool True), [])

    it "evaluates (< 10 0) = False" $ do
      eval [] example11 `shouldBe` Right (Just (ABool False), [])

    it "evaluates (eq? 0 0) = True" $ do
      eval [] example12 `shouldBe` Right (Just (ABool True), [])

    it "evaluates (eq? 10 0) = False" $ do
      eval [] example13 `shouldBe` Right (Just (ABool False), [])

  describe "define and variables" $ do
    it "evaluates (define x 5)" $ do
      eval [] (Define "x" (Atom 5)) `shouldBe` Right (Nothing, [("x", Atom 5)])

    it "evaluates symbol lookup after define" $ do
      let env = [("x", Atom 5)]
      eval env (Symbol "x") `shouldBe` Right (Just (Atom 5), env)

    it "evaluates exampleDefineTest" $ do
      eval [] exampleDefineTest `shouldBe` Right (Just (Atom 42), [("test", Atom 42)])

  describe "if expressions" $ do
    it "evaluates (if #t 1 2) = 1" $ do
      eval [] (If (ABool True) (Atom 1) (Atom 2)) `shouldBe` Right (Just (Atom 1), [])

    it "evaluates (if #f 1 2) = 2" $ do
      eval [] (If (ABool False) (Atom 1) (Atom 2)) `shouldBe` Right (Just (Atom 2), [])

    it "evaluates exampleTest with condition evaluation" $ do
      eval [] exampleTest `shouldBe` Right (Just (Atom 1), [])

  describe "atoms and bools" $ do
    it "evaluates raw Atom" $ do
      eval [] (Atom 42) `shouldBe` Right (Just (Atom 42), [])

    it "evaluates raw ABool" $ do
      eval [] (ABool True) `shouldBe` Right (Just (ABool True), [])

  describe "lambda expressions" $ do
    it "evaluates Lambda definition" $ do
      let lam = Lambda ["x"] (Atom 1)
      eval [] lam `shouldBe` Right (Just lam, [])

    it "evaluates lambda application (example14)" $ do
      eval [] example14 `shouldBe` Right (Just (Atom 5), [])

    it "evaluates exampleDefine" $ do
      let expected_lambda = Lambda ["x","y"] (List [Symbol "+", Symbol "x", Symbol "y"])
      eval [] exampleDefine `shouldBe` Right (Nothing, [("caca", expected_lambda)])
  ...
  describe "complex lambda examples" $ do
    ...
    it "applies exampleLambdaWithCall" $ do
      ...
      eval [] application `shouldBe` Right (Just (Atom 12), [])
